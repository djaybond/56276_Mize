...
nodebug void ScanIO()
{
    ...
    PingControl();
	CommFailure();
	DoutWrite(1,DinRead(302));//Well2 Running from RTU3
	DoutWrite(2,DinRead(402));//Well3 Running from RTU4
}

nodebug void CommFailure(void)
{
int i;
	for(i = 0; i < 2; i++)
	{
		...
		{
			++RTUComTimer[i];
			if(RTUComTimer[i]>1800) // 30 mins
			CommFailures[i] = TRUE;
		}
		else
		{
			RTUComTimer[i]=0;
			CommFailures[i] = FALSE;
			MQTT_CommFail[i] = 1;
		}
			DinWrite(10+i,CommFailures[i]);
	}
}
//}
...
//   DisplayMemoryUsage();
	...
	memset(MQTT_CommFail, 0, sizeof(MQTT_CommFail));
    memset(RTUComTimer, 0, sizeof(RTUComTimer));
    memset(CommFailures, 0, sizeof(CommFailures));
    ...
    strcpy(willTopic, "56276/RTU/2/NDEATH");
...
nodebug void ResetHard(void)
{
    ...
    */
    ...
    strcpy(topics[0], "56276/RTU/2/DI_STATUS");
    strcpy(topics[1], "56276/RTU/2/AI_STATUS");

    //No subscribe topics

    strcpy(theTopic,"56093/TemperatureF");
    strcpy(theMessage, "55.5");
    strcpy(clientID, "56093_RTU2");
...
nodebug void MonitorPublishItems()
{
	...
	char messageBit = 0;
	char RTU1_DI_DestAdd[] = "10201";
	char RTU1_AI_DestAdd[] = "30201";
	char length[3];
    ...
    case 1:
        if(timeToPublish[1] && !startPubOrSub) //Publish analogs on time only.
        {
            ...
            //strcpy(theMessage, "30201,2,");
			strcat(theMessage, RTU1_AI_DestAdd);
			strcat(theMessage,",");
			itoa(NUMBEROFAI,length);
			strcat(theMessage,length); //add the length
			strcat(theMessage,",");
            for(i=1; i <= NUMBEROFAI; i++)
            {
                itoa(AinRead(i), tempMess);
...
nodebug void ParseMessage(char *Topic, char *Message)
{
    ...
    */
    ...
    //printf("Message:%s\r\n",Message);
    //printf("Topic:%s\r\n",Topic);
    if(strcmp(subscribetopics[0],Topic) == 0) //"56093/RTU/3/DI_STATUS" Well 2

    {
        ...
        while(token != NULL)
        {
			...
			{
				DoutWrite(1,atoi(token));
				printf("Dout%d:%d\r\n",1,DoutRead(1));
			}
            token = strtok(NULL, s); // clear token
        }
    }

	if(strcmp(subscribetopics[1],Topic) == 0) //"56093/RTU/4/DI_STATUS"  Well 3
	...
	if(strcmp(subscribetopics[2],Topic) == 0) //"56093/RTU/1/FORCES" The message should look like
   {
	   i = 0;
       token = strtok(Message, s);
	   printf("Message:%s\r\n",Message);
	   ...
	   while( token != NULL )
	   {
		  ...
		  if(strcmp(token,"True") == 0 || strcmp(token," True") == 0)
			DoutWrite(2+i,1);
		  if(strcmp(token,"False") == 0 || strcmp(token," False") == 0)
			DoutWrite(2+i,0);
		  token = strtok(NULL, s);
	   }
	   printf("Dout3:%d Dout4:%d\r\n",DoutRead(3),DoutRead(4));
   }
    /*
       {
    	   i = 0;
    ...
    if(strcmp(subscribetopics[1],Topic) == 0) // "56093/RTU/4/DI_STATUS"
    {
        i = 0;
        token = strtok(Message, s);
        printf("Message:%s\r\n",Message);
...
*/
    /*
    if(strcmp(subscribetopics[2],Topic) == 0) //"56093/RTU/1/FORCES" VTS Forces The message would look something like this true,false
    {
        i = 0;
        token = strtok(Message, s);
        printf("Message:%s\r\n",Message);
        ...
        while( token != NULL )
        {
            ...
            {
                printf( " %s\n", token );
                DinWrite(1300+i-2,atoi(token));
                printf("Din%d:%d\r\n",i-2,DinRead(1300+i-2));
            }
            token = strtok(NULL, s);
        }
    }
...

       {
    	   ...
    	   while( token != NULL )
    	   {
    		   ...
    		   {
    			  printf( " %s\n", token );
    			  AinWrite(300+i-2,atof(token));
    			  printf("Ain%d:%.2f\r\n",300+i-2,AinRead(300+i-2));
    		   }
    		   token = strtok(NULL, s);
    	   }
       }
